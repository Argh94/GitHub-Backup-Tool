...
async function downloadAllRepositories() {
  ...
  if (links.length === 0) {
    showError(translations[document.documentElement.lang || 'en'].noReposAvailable || "No repositories available.");
    return;
  }
  
  const downloadAllButton = document.getElementById('download-all-button');
  const originalText = downloadAllButton.innerHTML;
  downloadAllButton.innerHTML = '<i class="fas fa-spinner fa-spin button-icon"></i><span>' + (document.documentElement.lang === 'fa' ? 'در حال دانلود...' : 'Downloading...') + '</span>';
  ...
  try {
    ...
    if (errorOccurred) {
      if (files.length === 0) {
        throw new Error(translations[document.documentElement.lang || 'en'].downloadFailed || 'Failed to download any repositories');
      } else if (!confirm(translations[document.documentElement.lang || 'en'].partialDownload || `Some repositories failed to download. Continue with ${files.length} successful downloads?`)) {
        throw new Error(translations[document.documentElement.lang || 'en'].downloadCancelled || 'Download cancelled');
      }
    }
